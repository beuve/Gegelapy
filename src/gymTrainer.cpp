...
GymTrainer::GymTrainer(
    ...
    uint64_t nb_iterations_per_policy_evaluation,
    uint64_t nb_iterations_per_policy_validation,
    uint64_t max_nb_actions_per_eval, double ratio_deleted_roots,
    uint64_t size_tournament, uint64_t nb_generations,
    size_t max_nb_evaluation_per_policy, size_t nb_iterations_per_job,
    size_t nb_registers, size_t nb_program_constant,
    std::string activation_function, bool do_validation,
    uint64_t step_validation, size_t nb_roots, double ratio_teams_over_actions,
    size_t max_init_outgoing_edges, double prob_change_action_class,
    double prob_action_edge_deletion, double prob_action_edge_addition,
    double prob_mutate_action_program, double prob_swap_action_program,
    size_t nb_action_edge_init, size_t max_outgoing_edges,
    double prob_edge_deletion, double prob_edge_addition,
    double prob_program_mutation, double prob_context_over_action_program,
    bool force_program_behavior_change_on_mutation,
    double prob_edge_destination_change, double prob_edge_destination_is_action,
    bool use_action_program, bool use_multi_action_program,
    bool team_access_all_actions, size_t max_program_size,
    size_t init_min_program_size, size_t init_max_program_size,
    double prob_delete, double prob_add, double prob_mutate, double prob_swap,
    double prob_constant_mutation, double prob_new_program,
    double min_const_value, double max_const_value)
    : env(env), currentGeneration(0), basicLogger(nullptr),
      policyStatsLogger(nullptr) {
  ...
  this->params.nbIterationsPerPolicyValidation =
      nb_iterations_per_policy_validation;
  ...
  this->params.mutation.tpg.probaContextOverActionProgram =
      prob_context_over_action_program;
  this->params.mutation.tpg.forceProgramBehaviorChangeOnMutation =
      force_program_behavior_change_on_mutation;
  this->params.mutation.tpg.pEdgeDestinationChange =
      prob_edge_destination_change;
  this->params.mutation.tpg.pEdgeDestinationIsAction =
      prob_edge_destination_is_action;
  ...
  for (auto *instr : instructions) {
    if (instr != nullptr) {
      ...
      std::cerr << "[GymTrainer] Warning: null instruction skipped."
                << std::endl;
    }
  }

  agent = std::make_unique<Learn::ParallelLearningAgent>(env, instructionSet,
                                                         params);
  agent->init();
...
void GymTrainer::save(const char *path, bool clean) {
  ...
  File::TPGGraphDotExporter dotExporter =
      File::TPGGraphDotExporter(path, *agent->getTPGGraph());
  dotExporter.print();
}

void GymTrainer::load(const char *path) {
  File::TPGGraphDotImporter dotImporter = File::TPGGraphDotImporter(
      path, agent->getTPGGraph()->getEnvironment(), *agent->getTPGGraph());
  dotImporter.importGraph();
}

void GymTrainer::step() { agent->trainOneGeneration(currentGeneration++); }
...
void GymTrainer::compile(const char *path) {
  ...
  std::unique_ptr<CodeGen::TPGGenerationEngine> tpggen =
      factory.create("generated_tpg", *agent->getTPGGraph(), c_src_dir);
