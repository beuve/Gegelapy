...
void add_instructions(py::module_ m) {
  ...
  [[maybe_unused]] auto lambdaInstruction1 =
      py::class_<Instructions::LambdaInstruction<double>,
                 Instructions::Instruction>(m, "LambdaInstruction1");
  [[maybe_unused]] auto lambdaInstruction2 =
      py::class_<Instructions::LambdaInstruction<double, double>,
                 Instructions::Instruction>(m, "LambdaInstruction2");

  auto f_add = [](double a, double b) -> double { return a + b; };
  auto i_add =
      Instructions::LambdaInstruction<double, double>(f_add, "$0 = $1 + $2;");
  m.attr("add") = py::cast(i_add);

  auto f_sub = [](double a, double b) -> double { return a - b; };
  auto i_sub =
      Instructions::LambdaInstruction<double, double>(f_sub, "$0 = $1 - $2;");
  m.attr("sub") = py::cast(i_sub);

  auto f_mul = [](double a, double b) -> double { return a * b; };
  auto i_mul =
      Instructions::LambdaInstruction<double, double>(f_mul, "$0 = $1 * $2;");
  m.attr("mul") = py::cast(i_mul);

  auto f_div = [](double a, double b) -> double { return a / b; };
  auto i_div =
      Instructions::LambdaInstruction<double, double>(f_div, "$0 = $1 / $2;");
  m.attr("div") = py::cast(i_div);

  auto f_max = [](double a, double b) -> double { return std::max(a, b); };
  auto i_max = Instructions::LambdaInstruction<double, double>(
      f_max, "$0 = (($1) < ($2)) ? ($2) : ($1);");
  m.attr("max") = py::cast(i_max);

  auto f_min = [](double a, double b) -> double { return std::min(a, b); };
  auto i_min = Instructions::LambdaInstruction<double, double>(
      f_min, "$0 = (($1) < ($2)) ? ($1) : ($2);");
